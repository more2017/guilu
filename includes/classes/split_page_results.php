...
class splitPageResults extends base {
  ...
  /* class functions */
    // display split-page-number-links  // display split-page-number-links// display split-page-number-links
  ...
  function display_links($max_page_links, $parameters = '', $outputAsUnorderedList = false) {
    ...
    $link = '<li><a aria-label="Previous" href="' . zen_href_link($_GET['main_page'], $parameters . $this->page_name . '=' . ($this->current_page_number - 1), $request_type) . '" title=" ' . PREVNEXT_TITLE_PREVIOUS_PAGE . ' "><span aria-hidden="true">&laquo;</span></a></li>';
	
	//// firstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirstfirst
    ...
    $link = 'yyyyyyyy<a href="' . zen_href_link($_GET['main_page'], $parameters . $this->page_name . '=' . (($cur_window_num - 1) * $max_page_links), $request_type) . '" title=" ' . sprintf(PREVNEXT_TITLE_PREV_SET_OF_NO_PAGE, $max_page_links) . ' ">...</a>';
    if ($cur_window_num > 1) {
      $display_links_string .= $link;
    ...
    for ($jump_to_page = 1 + (($cur_window_num - 1) * $max_page_links); ($jump_to_page <= ($cur_window_num * $max_page_links)) && ($jump_to_page <= $this->number_of_pages); $jump_to_page++) {
      if ($jump_to_page == $this->current_page_number) {
		...
		//////////currentccurrentcurrentcurrentcurrentcurrentcurrentcurrentcurrentcurrentcurrentcurrent
        $ul_elements .= '  <li class="current active">' . $jump_to_page . '</li>' . "\n";
        $counter_actual_page_links++;
      } else {
        $link = '<li><a href="' . zen_href_link($_GET['main_page'], $parameters . $this->page_name . '=' . $jump_to_page, $request_type) . '" title=" ' . sprintf(PREVNEXT_TITLE_PAGE_NO, $jump_to_page) . ' ">' . $jump_to_page . '</a></li>';
    ...
    // next button   next button   next button   next button   next button   
    if (($this->current_page_number < $this->number_of_pages) && ($this->number_of_pages != 1)) {
      $link = '<li><a aria-label="Next" href="' . zen_href_link($_GET['main_page'], $parameters . 'page=' . ($this->current_page_number + 1), $request_type) . '" title=" ' . PREVNEXT_TITLE_NEXT_PAGE . ' "><span aria-hidden="true">&raquo;</span></a></li>';
      $display_links_string .= $link;
