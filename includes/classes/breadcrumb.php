...
//    echo 'breadcrumb ' . $i . ' of ' . $n . ': ' . $this->_trail[$i]['title'] . '<br />';
      ...
      if (isset($this->_trail[$i]['link']) && zen_not_null($this->_trail[$i]['link']) && !$skip_link ) {
        ...
        if ($this->_trail[$i]['title'] == HEADER_TITLE_CATALOG) {
          $trail_string .= '  <a href="' . HTTP_SERVER . DIR_WS_CATALOG . '"><i class="fa fa-home fa-2x" aria-hidden="true"></i> ' . STORE_NAME . '</a>';
        } else {
      ...
      if (($i+1) < $n) $trail_string .= ' <i class="fa fa-angle-right fa-2x" aria-hidden="true"></i> ';
      $trail_string .= "\n";
    }
