...
//  $showFixedQuantityAmount = $products[$i]['quantity'] . zen_draw_hidden_field('cart_quantity[]', 1);
  ...
  $quantityField = zen_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="4" class="form-control cart_input_'.$products[$i]['id'].'"');
  ...
  //$buttonUpdate = 'xxxx'. ((SHOW_SHOPPING_CART_UPDATE == 1 or SHOW_SHOPPING_CART_UPDATE == 3) ? zen_image_submit(ICON_IMAGE_UPDATE, ICON_UPDATE_ALT) : '') . zen_draw_hidden_field('products_id[]', $products[$i]['id']);
  $buttonUpdate = '<input class="btn btn-primary btn-xs" type="submit" value="update">' . zen_draw_hidden_field('products_id[]', $products[$i]['id']);
