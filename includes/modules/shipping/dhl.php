 ...
 * @version $Id: dhl.php 6347 2007-05-20 19:46:59Z ajeh $
 */
/*

  USAGE
  By default, the module comes with support for 3 dhl.  This can be
  easily changed by editing the line below in the dhl constructor
  that defines $this->num_dhl.
  ...
  dhl.php.  A list of settings will appear to the right.  Click on the
  Edit button.
...
// class constructor
    function dhl() {
      ...
      $this->icon = 'includes/languages/english/modules/shipping/fastshipping.png';
      ...
      // CUSTOMIZE THIS SETTING FOR THE NUMBER OF DHL NEEDED
      $this->num_dhl = 4;
    }
...
// class methods
    ...
    $order_total_amount = $_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices() ;
      ...
      if ($dest_zone == 0) {
        ...
        $done = false;
        for ($i=0; $i<$size; $i+=2) {
          switch (MODULE_SHIPPING_DHL_METHOD) {
        	  case (MODULE_SHIPPING_DHL_METHOD == 'Weight'):
              if (round($shipping_weight,9) <= $dhl_table[$i]) {
                ...
                case (1):
                  ...
                  break;
                case (2):
                  $show_box_weight = ' (' . number_format($shipping_weight * $shipping_num_boxes,2) . MODULE_SHIPPING_DHL_TEXT_UNITS . ')';
                  break;
                default:
                  $show_box_weight = ' (' . $shipping_num_boxes . ' x ' . number_format($shipping_weight,2) . MODULE_SHIPPING_DHL_TEXT_UNITS . ')';
                  break;
                }

//                $shipping_method = MODULE_SHIPPING_DHL_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_DHL_TEXT_UNITS : '');
                $shipping_method = MODULE_SHIPPING_DHL_TEXT_WAY . ' ' . $dest_country . $show_box_weight;
                $done = true;
        if (strstr($dhl_table[$i+1], '%')) {
          $shipping = ($dhl_table[$i+1]/100) * $order_total_amount;
        } else {
          $shipping = $dhl_table[$i+1];
        }
                break;
      	    ...
      	    break;
        	  case (MODULE_SHIPPING_DHL_METHOD == 'Price'):
// shipping adjustment
              if (($_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices()) <= $dhl_table[$i]) {
                $shipping = $dhl_table[$i+1];
        ...
        if (strstr($dhl_table[$i+1], '%')) {
          ...
          $shipping = $dhl_table[$i+1];
	          ...
	          break;
        	  case (MODULE_SHIPPING_DHL_METHOD == 'Item'):
// shipping adjustment
              if (($total_count - $_SESSION['cart']->free_shipping_items()) <= $dhl_table[$i]) {
                $shipping = $dhl_table[$i+1];
                $shipping_method = MODULE_SHIPPING_DHL_TEXT_WAY . ' ' . $dest_country;
                $done = true;
        if (strstr($dhl_table[$i+1], '%')) {
        ...
        if ($shipping == -1) {
          ...
          switch (MODULE_SHIPPING_DHL_METHOD) {
              ...
              $shipping_cost = ($shipping) + constant('MODULE_SHIPPING_DHL_HANDLING_' . $dest_zone);
            break;
        	  case (MODULE_SHIPPING_DHL_METHOD == 'Item'):
      ...
      if (strstr(MODULE_SHIPPING_DHL_SKIPPED, $dest_country)) {
    ...
    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_DHL_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Calculation Method', 'MODULE_SHIPPING_DHL_METHOD', 'Weight', 'Calculate cost based on Weight, Price or Item?', '6', '0', 'zen_cfg_select_option(array(\'Weight\', \'Price\', \'Item\'), ', now())");
      ...
      for ($i = 1; $i <= $this->num_dhl; $i++) {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Countries', 'MODULE_SHIPPING_DHL_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are part of Zone " . $i . ".<br />Set as 00 to indicate all two character ISO country codes that are not specifically defined.', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_DHL_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights/prices. Example: 3:8.50,7:10.50,... Weight/Price less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.<br />You can end the last amount as 10000:7% to charge 7% of the Order Total', '6', '0', 'zen_cfg_textarea(', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_DHL_HANDLING_" . $i."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
      }
    }
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_DHL_STATUS', 'MODULE_SHIPPING_DHL_METHOD', 'MODULE_SHIPPING_DHL_TAX_CLASS', 'MODULE_SHIPPING_DHL_TAX_BASIS', 'MODULE_SHIPPING_DHL_SORT_ORDER', 'MODULE_SHIPPING_DHL_SKIPPED');
