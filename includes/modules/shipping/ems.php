 ...
 * @version $Id: ems.php 6347 2007-05-20 19:46:59Z ajeh $
 */
/*

  USAGE
  By default, the module comes with support for 3 ems.  This can be
  easily changed by editing the line below in the ems constructor
  that defines $this->num_ems.
  ...
  ems.php.  A list of settings will appear to the right.  Click on the
  Edit button.
...
*/
  ...
  class ems {
    var $code, $title, $description, $enabled, $num_ems;
...
// class constructor
    function ems() {
      ...
      $this->icon = 'includes/languages/english/modules/shipping/ems.png';
      ...
      // CUSTOMIZE THIS SETTING FOR THE NUMBER OF EMS NEEDED
      $this->num_ems = 4;
    }
...
// class methods
    ...
    $order_total_amount = $_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices() ;
      ...
      for ($i=1; $i<=$this->num_ems; $i++) {
        $countries_table = constant('MODULE_SHIPPING_EMS_COUNTRIES_' . $i);
        $countries_table = strtoupper(str_replace(' ', '', $countries_table));
        $country_ems = split("[,]", $countries_table);
        if (in_array($dest_country, $country_ems)) {
          $dest_zone = $i;
          break;
        }
        if (in_array('00', $country_ems)) {
          $dest_zone = $i;
          break;
        }
      }
      ...
      if ($dest_zone == 0) {
        ...
        $shipping = -1;
        $ems_cost = constant('MODULE_SHIPPING_EMS_COST_' . $dest_zone);

        $ems_table = split("[:,]" , $ems_cost);
        $size = sizeof($ems_table);
        $done = false;
        for ($i=0; $i<$size; $i+=2) {
          switch (MODULE_SHIPPING_EMS_METHOD) {
        	  case (MODULE_SHIPPING_EMS_METHOD == 'Weight'):
              if (round($shipping_weight,9) <= $ems_table[$i]) {
                $shipping = $ems_table[$i+1];
...
//                $shipping_method = MODULE_SHIPPING_EMS_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $shipping_weight . ' ' . MODULE_SHIPPING_EMS_TEXT_UNITS : '');
                ...
                $done = true;
        if (strstr($ems_table[$i+1], '%')) {
          $shipping = ($ems_table[$i+1]/100) * $order_total_amount;
        } else {
          $shipping = $ems_table[$i+1];
        }
                break;
...
// shipping adjustment
              if (($_SESSION['cart']->show_total() - $_SESSION['cart']->free_shipping_prices()) <= $ems_table[$i]) {
                $shipping = $ems_table[$i+1];
...
// shipping adjustment
              if (($total_count - $_SESSION['cart']->free_shipping_items()) <= $ems_table[$i]) {
                $shipping = $ems_table[$i+1];
                $shipping_method = MODULE_SHIPPING_EMS_TEXT_WAY . ' ' . $dest_country;
                $done = true;
        if (strstr($ems_table[$i+1], '%')) {
    ...
    function install() {
      global $db;
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_EMS_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
      ...
      for ($i = 1; $i <= $this->num_ems; $i++) {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_EMS_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights/prices. Example: 3:8.50,7:10.50,... Weight/Price less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.<br />You can end the last amount as 10000:7% to charge 7% of the Order Total', '6', '0', 'zen_cfg_textarea(', now())");
