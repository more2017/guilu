...
// qty box with add to cart button
        ...
        if ($listing->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          if (isset($_GET['manufacturers_id'])) {
            ...
            $lc_text = '<div class="imgwrapper"><a class="imglinks" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a></div>';
          }
        }
        break;
      }
    ...
    if ($product_listing_layout_style == 'columns') {
      ...
      $list_box_contents[$rows][$column] = array('params' => 'class="centerBoxContentsProducts centeredContent back  col-xs-6 col-sm-4 col-md-3 col-lg-3"' . ' ' . '',
                                                 'text'  => $lc_text);
      $column ++;
